...
public class FeedReaderTest {
    ...
    public void can_read_empty_forward() throws InterruptedException {
        assertThat(FeedReader.FORWARD.apply(endpoint, repository, "foobar", null, (commitment, location, direction, completed, entries) -> {
    ...
    public void can_read_multiple_pages_previous_location_forward() throws InterruptedException {
        ...
        assertThat(FeedReader.FORWARD.apply(endpoint, repository, "foobar", 0, (commitment, location, direction, completed, entries) -> {
            this.entries.addAll(entries);
            assertThat(direction).isEqualTo(FeedDirection.FORWARD);
            commitment.accept(null);
        }, () -> true)).contains(2);
    ...
    public void can_read_empty_backward() throws InterruptedException {
        assertThat(FeedReader.BACKWARD.apply(endpoint, repository, "foobar", null, (commitment, location, direction, completed, entries) -> {
            throw new AssertionError();
        }, () -> true)).isNotPresent();
        assertThat(repository.readCurrent("foobar")).isNotPresent();
    }
    ...
    public void can_read_multiple_pages_previous_location_backward() throws InterruptedException {
        endpoint.add("foo", "bar", "qux");
        repository.updateCurrent("foobar", 0, transaction -> { });
        assertThat(FeedReader.BACKWARD.apply(endpoint, repository, "foobar", 0, (commitment, location, direction, completed, entries) -> {
            this.entries.addAll(entries);
            assertThat(direction).isEqualTo(FeedDirection.BACKWARD);
            commitment.accept(null);
        }, () -> true)).contains(2);
    ...
    public void can_read_empty_backward_initial_only() throws InterruptedException {
        assertThat(FeedReader.BACKWARD_INITIAL_ONLY.apply(endpoint, repository, "foobar", null, (commitment, location, direction, completed, entries) -> {
            throw new AssertionError();
        }, () -> true)).isNotPresent();
        assertThat(repository.readCurrent("foobar")).isNotPresent();
    }
    ...
    public void can_read_multiple_pages_previous_location_backward_initial_only() throws InterruptedException {
        endpoint.add("foo", "bar", "qux");
        repository.updateCurrent("foobar", 0, transaction -> { });
        assertThat(FeedReader.BACKWARD_INITIAL_ONLY.apply(endpoint, repository, "foobar", 0, (commitment, location, direction, completed, entries) -> {
            this.entries.addAll(entries);
            assertThat(direction).isEqualTo(FeedDirection.FORWARD);
            commitment.accept(null);
        }, () -> true)).contains(2);
    ...
    public void can_read_empty_backward_once_only() throws InterruptedException {
        assertThat(FeedReader.BACKWARD_ONCE_ONLY.apply(endpoint, repository, "foobar", null, (commitment, location, direction, completed, entries) -> {
            throw new AssertionError();
        }, () -> true)).isNotPresent();
        assertThat(repository.readCurrent("foobar")).isNotPresent();
    }
    ...
    public void does_not_read_again_backwards_once_only() throws InterruptedException {
        endpoint.add("foo", "bar", "qux");
        assertThat(FeedReader.BACKWARD_ONCE_ONLY.apply(endpoint, repository, "foobar", 0, (commitment, location, direction, completed, entries) -> {
            this.entries.addAll(entries);
            assertThat(direction).isEqualTo(FeedDirection.FORWARD);
            commitment.accept(null);
        }, () -> true)).contains(0);
