...
public abstract class AbstractJdbcFeedPublisherTest<T extends DataSource> {
    ...
    @Test
    public void testPublishAndLookup() throws SQLException {
        try (Connection conn = dataSource.getConnection()) {
            conn.setAutoCommit(false);
            assertThat(publisher.initialize(conn)).isTrue();
    ...
    @Test
    public void testPagingForward() throws SQLException {
        try (Connection conn = dataSource.getConnection()) {
            conn.setAutoCommit(false);
            assertThat(publisher.initialize(conn)).isTrue();
    ...
    @Test
    public void testPagingBackward() throws SQLException {
        try (Connection conn = dataSource.getConnection()) {
            conn.setAutoCommit(false);
            assertThat(publisher.initialize(conn)).isTrue();
            publisher.publish(conn, -1, -2, -3);
    ...
    @Test
    public void testZeroSize() throws SQLException {
        try (Connection conn = dataSource.getConnection()) {
            conn.setAutoCommit(false);
            assertThat(publisher.initialize(conn)).isTrue();
            publisher.publish(conn, -1, -2, -3);
    ...
    public void testNegativeSize() throws SQLException {
        try (Connection conn = dataSource.getConnection()) {
            conn.setAutoCommit(false);
            assertThat(publisher.initialize(conn)).isTrue();
